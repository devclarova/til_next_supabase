...
- `/src/apis` 폴더 생성
- 참조: 경로 주의하셔야 합니다.
- 참조: 절대로 `/src/app/api` 폴더에 작성하면 Next가 작동됨
...
- 백엔드 연동 API가 완성되면 이제 Mutation을 만든다.

### 5.3. 왜 Mutation을 쓰라는거지?

- 현재 글을 등록하는데
- 현재 등록 중인 상태를 `isPending 개발자가 코딩으로 관리한다`
- 현재 글이 등록 완료되었는지 `response가 true이면 개발자가 코등으로 관리한다`
- 현재 글이 등록 실패했는지 `error 처리 개발자가 코딩으로 관리한다`
- 현재 글 등록 오류가 발생 시 `error 개발자가 코딩으로 관리한다`
...
- useQuery로 생성한 `특정 키`를 `전체 캐시 갱신`: 성능 상 좋지 않음
